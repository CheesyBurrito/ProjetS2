...
string Games::convertQuestionToString(int characteristicsSlected, int input) {
	string question = "";
	switch (characteristicsSlected)
	{
	case 0:
	...
	case 8:
		question += "Est-ce que votre personnage est ";
		question += characterManager.get_name_from_id(input);
		break;
	default:
		question = "ERROR : question generated by CPU is not valid";
		break;
...
bool Games::getAnswerToQuestion(int characteristicsSlected, int input, Player* player, Player* otherPlayer)
{
	switch (characteristicsSlected)
	{
	case 0:
