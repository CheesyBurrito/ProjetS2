/****************************************************************************
** Meta object code from reading C++ file 'FPGA.h'
**
** Created by: The Qt Meta Object Compiler version 67 (Qt 5.10.1)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "../FPGA.h"
#include <QtCore/qbytearray.h>
#include <QtCore/qmetatype.h>
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'FPGA.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 67
#error "This file was generated using the moc from 5.10.1. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
QT_WARNING_PUSH
QT_WARNING_DISABLE_DEPRECATED
struct qt_meta_stringdata_FPGA_t {
    QByteArrayData data[12];
    char stringdata0[137];
};
#define QT_MOC_LITERAL(idx, ofs, len) \
    Q_STATIC_BYTE_ARRAY_DATA_HEADER_INITIALIZER_WITH_OFFSET(len, \
    qptrdiff(offsetof(qt_meta_stringdata_FPGA_t, stringdata0) + ofs \
        - idx * sizeof(QByteArrayData)) \
    )
static const qt_meta_stringdata_FPGA_t qt_meta_stringdata_FPGA = {
    {
QT_MOC_LITERAL(0, 0, 4), // "FPGA"
QT_MOC_LITERAL(1, 5, 10), // "cardFailed"
QT_MOC_LITERAL(2, 16, 0), // ""
QT_MOC_LITERAL(3, 17, 6), // "cardOn"
QT_MOC_LITERAL(4, 24, 7), // "cardOff"
QT_MOC_LITERAL(5, 32, 16), // "detectedPhoneme1"
QT_MOC_LITERAL(6, 49, 16), // "detectedPhoneme2"
QT_MOC_LITERAL(7, 66, 16), // "detectedPhoneme3"
QT_MOC_LITERAL(8, 83, 16), // "detectedPhoneme4"
QT_MOC_LITERAL(9, 100, 8), // "readSlot"
QT_MOC_LITERAL(10, 109, 14), // "toggleReadMode"
QT_MOC_LITERAL(11, 124, 12) // "readAndPrint"

    },
    "FPGA\0cardFailed\0\0cardOn\0cardOff\0"
    "detectedPhoneme1\0detectedPhoneme2\0"
    "detectedPhoneme3\0detectedPhoneme4\0"
    "readSlot\0toggleReadMode\0readAndPrint"
};
#undef QT_MOC_LITERAL

static const uint qt_meta_data_FPGA[] = {

 // content:
       7,       // revision
       0,       // classname
       0,    0, // classinfo
      10,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       7,       // signalCount

 // signals: name, argc, parameters, tag, flags
       1,    0,   64,    2, 0x06 /* Public */,
       3,    0,   65,    2, 0x06 /* Public */,
       4,    0,   66,    2, 0x06 /* Public */,
       5,    0,   67,    2, 0x06 /* Public */,
       6,    0,   68,    2, 0x06 /* Public */,
       7,    0,   69,    2, 0x06 /* Public */,
       8,    0,   70,    2, 0x06 /* Public */,

 // slots: name, argc, parameters, tag, flags
       9,    0,   71,    2, 0x0a /* Public */,
      10,    0,   72,    2, 0x0a /* Public */,
      11,    0,   73,    2, 0x0a /* Public */,

 // signals: parameters
    QMetaType::Void,
    QMetaType::Void,
    QMetaType::Void,
    QMetaType::Void,
    QMetaType::Void,
    QMetaType::Void,
    QMetaType::Void,

 // slots: parameters
    QMetaType::Void,
    QMetaType::Void,
    QMetaType::Void,

       0        // eod
};

void FPGA::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        FPGA *_t = static_cast<FPGA *>(_o);
        Q_UNUSED(_t)
        switch (_id) {
        case 0: _t->cardFailed(); break;
        case 1: _t->cardOn(); break;
        case 2: _t->cardOff(); break;
        case 3: _t->detectedPhoneme1(); break;
        case 4: _t->detectedPhoneme2(); break;
        case 5: _t->detectedPhoneme3(); break;
        case 6: _t->detectedPhoneme4(); break;
        case 7: _t->readSlot(); break;
        case 8: _t->toggleReadMode(); break;
        case 9: _t->readAndPrint(); break;
        default: ;
        }
    } else if (_c == QMetaObject::IndexOfMethod) {
        int *result = reinterpret_cast<int *>(_a[0]);
        {
            typedef void (FPGA::*_t)();
            if (*reinterpret_cast<_t *>(_a[1]) == static_cast<_t>(&FPGA::cardFailed)) {
                *result = 0;
                return;
            }
        }
        {
            typedef void (FPGA::*_t)();
            if (*reinterpret_cast<_t *>(_a[1]) == static_cast<_t>(&FPGA::cardOn)) {
                *result = 1;
                return;
            }
        }
        {
            typedef void (FPGA::*_t)();
            if (*reinterpret_cast<_t *>(_a[1]) == static_cast<_t>(&FPGA::cardOff)) {
                *result = 2;
                return;
            }
        }
        {
            typedef void (FPGA::*_t)();
            if (*reinterpret_cast<_t *>(_a[1]) == static_cast<_t>(&FPGA::detectedPhoneme1)) {
                *result = 3;
                return;
            }
        }
        {
            typedef void (FPGA::*_t)();
            if (*reinterpret_cast<_t *>(_a[1]) == static_cast<_t>(&FPGA::detectedPhoneme2)) {
                *result = 4;
                return;
            }
        }
        {
            typedef void (FPGA::*_t)();
            if (*reinterpret_cast<_t *>(_a[1]) == static_cast<_t>(&FPGA::detectedPhoneme3)) {
                *result = 5;
                return;
            }
        }
        {
            typedef void (FPGA::*_t)();
            if (*reinterpret_cast<_t *>(_a[1]) == static_cast<_t>(&FPGA::detectedPhoneme4)) {
                *result = 6;
                return;
            }
        }
    }
    Q_UNUSED(_a);
}

QT_INIT_METAOBJECT const QMetaObject FPGA::staticMetaObject = {
    { &QObject::staticMetaObject, qt_meta_stringdata_FPGA.data,
      qt_meta_data_FPGA,  qt_static_metacall, nullptr, nullptr}
};


const QMetaObject *FPGA::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->dynamicMetaObject() : &staticMetaObject;
}

void *FPGA::qt_metacast(const char *_clname)
{
    if (!_clname) return nullptr;
    if (!strcmp(_clname, qt_meta_stringdata_FPGA.stringdata0))
        return static_cast<void*>(this);
    return QObject::qt_metacast(_clname);
}

int FPGA::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QObject::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 10)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 10;
    } else if (_c == QMetaObject::RegisterMethodArgumentMetaType) {
        if (_id < 10)
            *reinterpret_cast<int*>(_a[0]) = -1;
        _id -= 10;
    }
    return _id;
}

// SIGNAL 0
void FPGA::cardFailed()
{
    QMetaObject::activate(this, &staticMetaObject, 0, nullptr);
}

// SIGNAL 1
void FPGA::cardOn()
{
    QMetaObject::activate(this, &staticMetaObject, 1, nullptr);
}

// SIGNAL 2
void FPGA::cardOff()
{
    QMetaObject::activate(this, &staticMetaObject, 2, nullptr);
}

// SIGNAL 3
void FPGA::detectedPhoneme1()
{
    QMetaObject::activate(this, &staticMetaObject, 3, nullptr);
}

// SIGNAL 4
void FPGA::detectedPhoneme2()
{
    QMetaObject::activate(this, &staticMetaObject, 4, nullptr);
}

// SIGNAL 5
void FPGA::detectedPhoneme3()
{
    QMetaObject::activate(this, &staticMetaObject, 5, nullptr);
}

// SIGNAL 6
void FPGA::detectedPhoneme4()
{
    QMetaObject::activate(this, &staticMetaObject, 6, nullptr);
}
QT_WARNING_POP
QT_END_MOC_NAMESPACE
